...
WebInterface::WebInterface(WaveformGenerator& wave, AlarmManager& alarm,
  ...
  : server(80), events("/events"), waveform(wave), alarms(alarm),
    device(dev), storage(stor), sensor(sens), currentCO2Value(0), lastDataUpdate(0) {}

void WebInterface::setUsbModeCallback(std::function<void(bool)> cb) {
  usbModeCallback = cb;
}
...
void WebInterface::setupRoutes() {
  ...
  server.on("/api/settings", HTTP_GET, [this](AsyncWebServerRequest *request){
    ...
    doc["useCocoSensor"] = waveform.isUsingCocoSensor();
    doc["waveformType"] = (uint8_t)waveform.getWaveformType();
    doc["usbMode"] = device.isUsbProtocolMode() ? "protocol" : "debug";
    doc["continuousMode"] = device.isContinuousMode();
    doc["sensorAvailable"] = sensor.isAvailable();
    doc["sensorMeasuring"] = sensor.isMeasurementRunning();
  ...
  server.on("/api/settings", HTTP_POST, [](AsyncWebServerRequest *request){},
    ...
    if (doc.containsKey("useCocoSensor")) waveform.setUseCocoSensor(doc["useCocoSensor"]);
    if (doc.containsKey("waveformType")) {
      uint8_t wt = doc["waveformType"];
      waveform.setWaveformType(static_cast<WaveformGenerator::WaveformType>(wt));
    }
    if (doc.containsKey("usbMode")) {
      bool protocolMode = (String(doc["usbMode"].as<const char*>()) == "protocol");
      if (usbModeCallback) usbModeCallback(protocolMode);
    }
    if (doc.containsKey("streaming")) {
      bool streaming = doc["streaming"];
      if (streaming) device.startContinuousMode();
      else device.stopContinuousMode();
    }
...
input[type="number"],input[type="range"],select{padding:8px;border:1px solid #dadce0;border-radius:4px;font-size:14px}
...
.status-badge.protocol{background:#e8f0fe;color:#1967d2}
...
.hint{color:#5f6368;font-size:12px;margin-top:8px}
...
</div><div id="alarmStatus" class="alarm"><strong>ALARM:</strong> CO2 out of range!</div></div>

<div class="card"><h2>USB Port Mode</h2>
<div class="control-group"><label>USB Mode:</label>
<select id="usbMode" onchange="setUsbMode(this.value)">
<option value="debug">Debug / CLI</option>
<option value="protocol">Protocol Mirror</option>
</select>
<span class="status-badge" id="usbModeBadge" style="margin-left:10px">DEBUG</span>
</div>
<p class="hint">Protocol mode: USB mirrors Serial1 Respironics data and accepts protocol commands. CLI unavailable in protocol mode.</p>
<div class="control-group" style="margin-top:15px"><label>Protocol Streaming:</label>
<button id="streamBtn" onclick="toggleStreaming()">Start Streaming</button>
<span class="status-badge" id="streamBadge" style="margin-left:10px">IDLE</span>
</div>
<p class="hint">Controls whether Respironics waveform packets are being generated on Serial1 (and USB if in protocol mode).</p>
</div>

<div class="card"><h2>Waveform Control</h2>
<div class="control-group"><label>CO2 Source:</label>
<select id="co2Source" onchange="updateSettings()">
<option value="sim">Simulation</option>
<option value="coco">CoCo Sensor</option>
</select>
<span class="status-badge" id="sensorBadge" style="margin-left:10px">--</span>
</div>
<div class="control-group"><label>Waveform Type:</label>
<select id="waveformType" onchange="updateSettings()">
<option value="0">Sine Wave</option>
<option value="1">Capnogram</option>
</select></div>
...
<button onclick="loadConfig()" class="secondary">Load from EEPROM</button></div>
</div>
<script>
...
function updateDisplay(data){
  document.getElementById('currentCO2').textContent=data.co2.toFixed(2);
  ...
  let sBadge=document.getElementById('streamBadge');let sBtn=document.getElementById('streamBtn');
  if(data.mode==='CONTINUOUS'){sBadge.textContent='RUNNING';sBadge.className='status-badge active';sBtn.textContent='Stop Streaming';}
  else{sBadge.textContent='IDLE';sBadge.className='status-badge inactive';sBtn.textContent='Start Streaming';}

  let usbBadge=document.getElementById('usbModeBadge');
  if(data.usbMode==='protocol'){usbBadge.textContent='PROTOCOL';usbBadge.className='status-badge protocol';}
  else{usbBadge.textContent='DEBUG';usbBadge.className='status-badge active';}

  let sensorBadge=document.getElementById('sensorBadge');
  if(data.sensorAvailable){sensorBadge.textContent='Connected';sensorBadge.className='status-badge active';}
  else{sensorBadge.textContent='Not Found';sensorBadge.className='status-badge inactive';}
...
function updateSettings(){
  let settings={
    ...
    baseline:parseFloat(document.getElementById('base').value),
    phase:parseFloat(document.getElementById('phase').value),
    alarmHigh:parseFloat(document.getElementById('alarmHigh').value),
    alarmLow:parseFloat(document.getElementById('alarmLow').value),
    alarmHighEnabled:document.getElementById('alarmHighEn').checked,
    alarmLowEnabled:document.getElementById('alarmLowEn').checked,
    useCocoSensor:document.getElementById('co2Source').value==='coco',
...
function setUsbMode(mode){
  fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({usbMode:mode})})
  .then(r=>r.json()).then(data=>console.log('USB mode set to '+mode));
}

function toggleStreaming(){
  let isRunning=document.getElementById('streamBadge').textContent==='RUNNING';
  fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({streaming:!isRunning})})
  .then(r=>r.json()).then(data=>console.log('Streaming toggled'));
}
...
fetch('/api/settings').then(r=>r.json()).then(data=>{
  document.getElementById('amp').value=data.amplitude;
  document.getElementById('ampVal').textContent=data.amplitude;
  document.getElementById('freq').value=data.frequency;
  document.getElementById('freqVal').textContent=data.frequency;
  document.getElementById('base').value=data.baseline;
  document.getElementById('baseVal').textContent=data.baseline;
  document.getElementById('phase').value=data.phase;
  document.getElementById('phaseVal').textContent=data.phase;
  document.getElementById('alarmHigh').value=data.alarmHigh;
  document.getElementById('alarmLow').value=data.alarmLow;
  document.getElementById('alarmHighEn').checked=data.alarmHighEnabled;
  document.getElementById('alarmLowEn').checked=data.alarmLowEnabled;
  document.getElementById('co2Source').value=data.useCocoSensor?'coco':'sim';
  document.getElementById('waveformType').value=data.waveformType;
  document.getElementById('usbMode').value=data.usbMode;
});
